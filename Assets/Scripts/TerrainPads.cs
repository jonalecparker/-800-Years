...
// first use) by more than MaxDeviation (3m, user-tuned 2026-08-07). Every operation clamps against
// pristine, not against the current state, so repeated carving cannot
// creep past the law. The hill's strategic shape is immutable; only its
// surface is negotiable. (Design call 2026-08-07: shaping is a small
// bounded facility for seating buildings and benching paths, never a
// sculpting feature. Labor pricing arrives with the economy.)
//
...
//
// Edits go to a runtime COPY of the TerrainData (play-mode writes to
// the authored asset silently persist — it is never touched). Ground
// under EXISTING terrain-storey walls is load-bearing (footings never
// resample), so live ops stop short of them (KeepOut). On load nothing
// exists yet when ops replay, and every wall re-foots afterwards.
public static class TerrainPads
{
    ...
    static float[,,] pristineAlpha;
    static TerrainData runtime;

    static Terrain Prepare()
    {
        Terrain terrain = Terrain.activeTerrain;
        if (terrain == null)
            return null;
        if (runtime == null || terrain.terrainData != runtime)
        {
    ...
    // earth and a flattened pad grows back to grass.
    static void RefreshSurface(Terrain terrain, int hc0, int hr0, int hc1, int hr1)
    {
        if (runtime.alphamapLayers != 2)
            return;
        ...
        int hres = runtime.heightmapResolution;
        int ares = runtime.alphamapWidth;
        ...
        if (a1 <= a0 || b1 <= b0)
            return;
        var alphas = new float[b1 - b0 + 1, a1 - a0 + 1, 2];
        for (int r = b0; r <= b1; r++)
        {
            for (int c = a0; c <= a1; c++)
            {
                float steep = runtime.GetSteepness(
                    (float)c / (ares - 1), (float)r / (ares - 1));
                float dirt = Mathf.InverseLerp(slopeStart, slopeFull, steep);
                alphas[r - b0, c - a0, 0] = 1f - dirt;
                alphas[r - b0, c - a0, 1] = dirt;
            }
        }
        runtime.SetAlphamaps(a0, b0, alphas);
    }
    ...
    {
        Terrain terrain = Prepare();
        if (terrain == null)
            return 0f;
        return SampleField(pristine, x, z, terrain);
    }

    // Bilinear world-Y sample of a stored heightmap snapshot.
    static float SampleField(float[,] field, float x, float z, Terrain terrain)
    {
        int res = runtime.heightmapResolution;
        Vector3 size = runtime.size;
    ...
    // mouse releases. The first PreviewBench snapshots the current
    // (post-ops) heightmap; each call reverts the last previewed region
    // from that snapshot, then applies the candidate bench WITHOUT
    // recording it. A commit is CancelPreview (put the snapshot back)
    // followed by the ordinary Bench() — the same door a direct commit
    // uses, so what previews is exactly what commits.
    // ------------------------------------------------------------------
    static float[,] previewBase;
    static int pvC0, pvR0, pvC1, pvR1;
    ...
    public static void PreviewBench(Vector3 a, Vector3 b, float width, float ya, float yb)
    {
        ...
        {
            int res = runtime.heightmapResolution;
            previewBase = runtime.GetHeights(0, 0, res, res);
        }
        RevertPreview(terrain);
        if (FlatLen(a, b) < 0.5f)
            return;
        previewApplied = Reshape(
            SegBounds(a, b, width * 0.5f),
            w => SegmentDistXZ(w, a, b) <= width * 0.5f,
            w => Mathf.Lerp(ya, yb, SegmentT(w, a, b)),
            out pvC0, out pvR0, out pvC1, out pvR1);
    }
    ...
    public static float PrePreviewGroundAt(float x, float z)
    {
        ...
        return SampleField(previewBase, x, z, terrain);
    }
    ...
    static void RevertPreview(Terrain terrain)
    {
        if (!previewApplied || previewBase == null)
            return;
        int w = pvC1 - pvC0 + 1, h = pvR1 - pvR0 + 1;
        var patch = new float[h, w];
        for (int r = 0; r < h; r++)
            for (int c = 0; c < w; c++)
                patch[r, c] = previewBase[pvR0 + r, pvC0 + c];
        runtime.SetHeights(pvC0, pvR0, patch);
        RefreshSurface(terrain, pvC0, pvR0, pvC1, pvR1);
    ...
    public static void ResetToPristine()
    {
        ...
        previewBase = null;
        previewApplied = false;
        if (runtime != null && pristine != null)
            runtime.SetHeights(0, 0, pristine);
        if (runtime != null && pristineAlpha != null)
            runtime.SetAlphamaps(0, 0, pristineAlpha);
    }

    // ------------------------------------------------------------------
    // The one sample loop every operation runs through. The law is
    // enforced HERE — per sample, against pristine — so no caller can
    // dig past it, whatever level it asks for.
    // ------------------------------------------------------------------
    ...
    {
        return Reshape(bounds, covers, levelAt, out _, out _, out _, out _);
    }
        ...
        out int oc0, out int or0, out int oc1, out int or1)
    {
        ...
        float[,] heights = runtime.GetHeights(c0, r0, c1 - c0 + 1, r1 - r0 + 1);
        for (int r = r0; r <= r1; r++)
        {
            for (int c = c0; c <= c1; c++)
            {
                ...
                if (held)
                    continue;
                float natural = origin.y + pristine[r, c] * size.y;
        ...
        runtime.SetHeights(c0, r0, heights);
        RefreshSurface(terrain, c0, r0, c1, r1);
        return true;
    }
